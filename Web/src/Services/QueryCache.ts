...
const queries: Record<string, AppQuery> = {
  published: {
    ...
    queryFn: (key, project) => FC.service('uploads').find({ query: { status: 'released', project, $sort: { releasedAt: -1 } } }),
  ...
  updateSizes: {
    ...
    queryFn: (key, uploadId) => uploadId
      ? FC.service('utils').get('updateSizes', { query: { uploadId } })
      : null,
    noInitalPrefetch: true
  },
  patches: {
    config: queryConfig.frequent,
    queryFn: (key, project) => FC.service('patches').find({
  ...
  patchJobs: {
    config: queryConfig.frequent,
    queryFn: (key, project) => FC.service('patch-jobs').find({
...
export const useCQuery = <T = DynamicData>(queryKey: QueryKeyValue): UseQueryResult<T> => {
  ...
  return useQuery({ queryKey: asQueryKey(queryKey), queryFn: () => runQuery(queryKey, queryFn) as T, ...config, enabled: FC.isReady() })
}
